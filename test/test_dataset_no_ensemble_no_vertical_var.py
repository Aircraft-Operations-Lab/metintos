...
class TestDatasetHandler(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        file = os.path.join(files_path, 'local_forecast_xy_no_ensemble_only_steps_x_y.nc')
        ...
        cls.dsn = cls.dsh.get_optical_flow_interpolated_dataset(cls.cg.axes, ['t'])

    def test_dataset(self):
        # check ds
        list_variables = list(self.ds.variables.keys())
        self.assertEqual(len(list_variables), 8, 'Variables ds from xr.open_mfdataset are not equal')
    ...
    def test_datasetHandler(self):
        ...
        self.assertEqual(len(list_variables), 8, 'Variables ds in dsh from xr.open_mfdataset are not equal')
    ...
    def test_optical_flow(self):
        ...
        example_of = self.dsn.t.values[1, 2, 5]
        self.assertEqual(round(float(example_of), 9), 1.821505666,
